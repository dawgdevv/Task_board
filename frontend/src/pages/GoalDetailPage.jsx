...
const ActionPlanCard = React.memo(({ list, index, isSelected, onSelectList }) => {
  return (
    <div
    ...
    >
      <h4 className="text-lg font-semibold text-[var(--ctp-text)] truncate pr-2 mb-3">{list.name}</h4>
      <p className="text-[var(--ctp-subtext1)] text-sm mb-3">Created: {new Date(list.createdAt).toLocaleDateString()}</p>
      {isSelected && (
...
const GoalDetailPage = () => {
  ...
  const handleLogout = () => { // Not performance critical for re-renders, but fine to keep as is or memoize if desired
    localStorage.removeItem("token");
    localStorage.removeItem("user");
    navigate("/login");
  };

  const handleListSelect = useCallback((index) => setSelectedListIndex(index), []);
  ...
  const handleListDelete = useCallback(async (deletedListId) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
      ...
      if (response.ok) {
        ...
        if (selectedTaskListForModal && selectedTaskListForModal._id === deletedListId) {
  ...
  }, [taskLists, selectedListIndex, selectedTaskListForModal, fetchGoalData, handleCloseTaskListModal]);

  const getPriorityColor = (priority) => {
    switch (priority) {
      case "high": return "bg-[var(--ctp-red)]/30 text-[var(--ctp-red)]";
      case "medium": return "bg-[var(--ctp-yellow)]/30 text-[var(--ctp-yellow)]";
      case "low": return "bg-[var(--ctp-green)]/30 text-[var(--ctp-green)]";
      default: return "bg-[var(--ctp-surface1)] text-[var(--ctp-subtext0)]";
    }
  };
  ...
  return (
    <div className="min-h-screen"> {/* bg-gray-900 removed */}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="card p-6 mb-8"> {/* Use .card class */}
          <div className="flex justify-between items-start mb-4">
            <div className="flex-1">
              ...
              {goal.description && (
                <div className="text-[var(--ctp-subtext1)] mb-3">
              ...
              <div className="flex flex-wrap gap-3 items-center">
                ...
                >
                 ...
                 <span className={`px-3 py-1 rounded-full text-sm font-semibold ${daysRemaining >=0 ? 'bg-[var(--ctp-green)]/30 text-[var(--ctp-green)]' : 'bg-[var(--ctp-red)]/30 text-[var(--ctp-red)]'}`}>
                    {daysRemaining >= 0
              ...
              <div className="flex flex-col gap-2">
                ...
                >
                  <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                  </svg>
                  Edit Goal
                </button>
                <button
        ...
        <div className="mb-8">
          ...
          {showCreateList && (
            <div className="card mb-4 p-4"> {/* Use .card */}
          ...
          {taskLists.length > 0 ? (
            <div className="mb-6">
              <div className="flex items-center space-x-3 mb-4">
                <h3 className="text-lg font-medium text-[var(--ctp-text)]">Select Action Plan:</h3>
                <span className="text-sm text-[var(--ctp-subtext0)]">
                  ({taskLists.length} action plan{taskLists.length !== 1 ? "s" : ""})
                </span>
              </div>
              ...
              <p className="text-xs text-[var(--ctp-overlay0)] mt-2">
                💡 Scroll horizontally to view all action plans. Click to select and view tasks below.
              </p>
            </div>
          ) : (
            <div className="text-center py-8 card"> {/* Use .card */}
        ...
        {selectedListIndex >= 0 && taskLists[selectedListIndex] && (
          <div className="mb-8">
            <div className="card shadow-lg"> {/* Use .card */}
              <div className="bg-[var(--ctp-crust)] px-6 py-4 border-b border-[var(--ctp-surface1)] rounded-t-lg">
                <div className="flex justify-between items-center">
                  ...
                  <button
                    onClick={() => handleOpenTaskListModal(taskLists[selectedListIndex])}
        ...
        <div className="fixed inset-0 bg-[var(--ctp-base)] bg-opacity-80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-[var(--ctp-mantle)] rounded-lg shadow-2xl border border-[var(--ctp-surface1)] w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
            <div className="bg-[var(--ctp-crust)] px-6 py-4 border-b border-[var(--ctp-surface1)] flex justify-between items-center">
              <div className="flex items-center space-x-3">
                <h3 className="text-xl font-bold text-[var(--ctp-text)]">{selectedTaskListForModal.name}</h3>
            ...
            <div className="bg-[var(--ctp-crust)] px-6 py-3 border-t border-[var(--ctp-surface1)] flex justify-end">
              <button onClick={handleCloseTaskListModal} className="button-secondary px-4 py-2">
                Close
              </button>
            </div>
          </div>
      ...
      {showEditGoal && (
        <EditGoalModal
          ...
          onUpdated={async () => { await fetchGoalData(); setShowEditGoal(false); }}
        />
      )}

      <TimeTracker
        goalId={goalId}
        taskListId={selectedListIndex >= 0 ? taskLists[selectedListIndex]?._id : null}
