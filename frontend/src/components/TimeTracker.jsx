...
const TimeTracker = ({
  ...
  return (
    <div className="fixed inset-0 bg-[var(--ctp-base)] bg-opacity-80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="card w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl">
        {" "}
        {/* Use .card */}
        <div className="bg-[var(--ctp-crust)] px-6 py-4 border-b border-[var(--ctp-surface1)] rounded-t-lg flex justify-between items-center">
          <h3 className="text-lg font-semibold text-[var(--ctp-text)] flex items-center">
            <svg
              className="w-5 h-5 mr-2 text-[var(--ctp-sky)]"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
        ...
        <div className="p-6">
          <div className="text-center mb-6">
            ...
            <div className="flex justify-center gap-3 mb-6 flex-wrap">
              {!isRunning ? (
                <button
                  onClick={handleStart}
                  className="bg-[var(--ctp-green)] text-[var(--ctp-base)] px-4 py-2 rounded-md flex items-center hover:opacity-80 transition-opacity font-medium"
                >
                  <svg
                ...
                <button
                  onClick={handlePause}
                  className="bg-[var(--ctp-yellow)] text-[var(--ctp-base)] px-4 py-2 rounded-md flex items-center hover:opacity-80 transition-opacity font-medium"
                >
                  <svg
                    className="w-4 h-4 mr-2"
                    fill="currentColor"
              ...
              <button
                onClick={handleStop}
                disabled={elapsed === 0}
                className="button-danger px-4 py-2 flex items-center disabled:bg-[var(--ctp-surface1)] disabled:text-[var(--ctp-overlay0)] font-medium"
              >
                <svg
                  className="w-4 h-4 mr-2"
                  fill="currentColor"
              ...
              <button
                onClick={handleReset}
                className="button-secondary px-4 py-2 font-medium"
              >
                Reset
              </button>
            </div>
          ...
          <div className="border-t border-[var(--ctp-surface1)] pt-4">
            <button
              ...
              className="text-[var(--ctp-peach)] text-sm hover:text-[var(--ctp-flamingo)] mb-3"
            >
            ...
            {showManualEntry && (
              <form onSubmit={handleManualSubmit} className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-sm text-[var(--ctp-subtext0)] mb-1">
                      Hours
                    </label>
                    <input
                      type="number"
                ...
                <button
                  ...
                  className="button-primary w-full py-2 disabled:bg-[var(--ctp-surface1)] disabled:text-[var(--ctp-overlay0)]"
                >
                  Log Manual Time
                </button>
              </form>
